...
if s:syntax_block_definitions
   ...
   syn match matlabRepeat		'\<\%(parfor\|for\|while\)\>'					| " removed 'do' statement
   syn match matlabObjectOriented	'\<\%(classdef\|methods\|properties\|events\|enumeration\)\>'	| " removed matlabScope
   syn keyword matlabScope		persistent global						| " added
   ...
   syn keyword matlabRepeat		parfor for while						| " no do
   syn keyword matlabObjectOriented	classdef methods properties events enumeration			| " removed matlabScope
   syn keyword matlabScope		persistent global						| " added
...
syn match matlabProceduralEnd		'\<end\>' 								
"     ... Inside an Index Expression ...
syn match matlabIndexEnd		'\%([-+{\*\:(\/\[]\s*\)\@<=\<end\>' 					
syn match matlabIndexEnd		'\<end\>\%(\s*[-+}\:\*\/)\]]\)\@=' 					
...
" --- Todo words ---
syn match matlabTodo			"\c\<\%(todo\|notes\?\|fixme\|xxx\)\%(\s\d\+\|\d*\)\>:\=" 	contained	
...
syn match matlabArithmeticOperator	"\.\=[*/\\^]"					
syn match matlabError			"\%(\.\=[*/\\^]\)\{2,}"				
" --- Logical Operators ---
syn match matlabBoolean			'\<true\>\|\<false\>'
"syn keyword matlabBoolean		true false
syn match matlabLogicalOperator		"[&|~]"
syn match matlabError			"\%(&|\||&\)"					
syn match matlabError			"\%([&|]\)\{3,}"				
" --- Function Handle ---
syn match matlabError			"@"						
syn match matlabFunctionHandle		"@\%(\s*[(A-Za-z]\)\@="				
" --- Meta-Class Operator ---
syn match matlabError			"?"						
syn match matlabQuestionMark		"?\%(\s*[A-Za-z]\)\@="				
" --- Structure Dot ---
syn match matlabStructDot		"\%(\w\|)\|}\)\@<=\.\%(\a\|(\)\@="		
" --- Colon Operator ---
syn match matlabColon			":"						
" --- Line Continuation ---
...
syn match matlabError			'\<_\w*'		
"     ... Word Containing Unused Characters ...
syn match matlabError			'\w*[`#$"]\w*'		

" =========== "Strings" ===========
" --- Basic String ---
syn region matlabString			start=+'+ skip=/\%(''\)\+/ end=+'+	oneline	contains=@Spell,matlabTexFormat,matlabSprintfFormat	
" --- Incomplete String ---
syn match matlabIncompleteString	"'\%([^']\|''\)*"			contains=matlabString,matlabIncompleteString			

" --- Sprinf Formats ---
"     ... Special Chars ...
syn match matlabSprintfFormat		"\\[abfnrtv]"			contained				
syn match matlabSprintfFormat		"\\x[\x\o]\+" 			contained				
syn match matlabSprintfFormat		"\%(%%\|\\\\\)" 		contains=matlabTexFormat contained	
"     ... Input Formats ...
syn match matlabSprintfFormat		"%\%(\d\+\$\)\?[-+ 0#]\?\%(\d\+\|\*\|\*\d\+\$\)\?\%(\.\d\+\|\.\*\|\.\*\d\+\$\)\?\%([cdeEfgGiosuqxX]\|[bt][ouxX]\|[hl][diouxX]\)"	 contained 
"syn match matlabSprintfFormat		"%[-+ 0#]\?\%(\d\+\)\?\%(\.\d\+\)\?[cdeEfgGosuxX]" contained	(simple sprintf)

" --- TeX Keywords ---
syn match matlabTexFormat		"\\\%([Aa]lpha\|[Bb]eta\|[Gg]amma\|[Dd]elta\|[Ee]psilon\|[Zz]eta\|[Ee]ta\|[Tt]heta\|[Ii]ota\|[Kk]appa\|[Ll]ambda\)" 			contained 
syn match matlabTexFormat		"\\\%([Mm]u\|[Nn]u\|[Xx]i\|[Pp]i\|[Rr]ho\|[Ss]igma\|[Tt]au\|[Uu]psilon\|[Pp]hi\|[Cc]hi\|[Pp]si\|[Oo]mega\)" 				contained 
syn match matlabTexFormat		"\\\%(sim\|leq\|infty\|clubsuit\|diamondsuit\|heartsuit\|spadesuit\|leftrightarrow\|vartheta\|leftarrow\|uparrow\|rightarrow\)" 	contained 
syn match matlabTexFormat		"\\\%(downarrow\|circ\|pm\|geq\|propto\|forall\|partial\|exists\|bullet\|varsigma\|div\|cong\|neq\|equiv\|approx\|aleph\|Im\)" 		contained 
syn match matlabTexFormat		"\\\%(Re\|wp\|otimes\|oplus\|oslash\|cap\|cup\|supseteq\|supset\|subseteq\|subset\|int\|in\|rfloor\|lceil\|nabla\|lfloor\|cdot\)"	contained 
syn match matlabTexFormat		"\\\%(ldots\|perp\|neg\|prime\|wedge\|times\|0\|rceil\|surd\|mid\|vee\|varpi\|copyright\|langle\|rangle\)"				contained 
...
" --- Cells ---
syn match matlabTitle			'\%(^\s*\)\@<=%%\%($\|\s.*$\)'	contains=matlabTodo,@Spell				
" TODO - matlabTitleBlock
" --- M-Lint ---
syn match matlabMLint			'%#ok\%(<\*\?\u\+\%(,\u\+\)*>\)\?' contained  						
" --- Comments ---
"     ... Line Continuation ...
syn match matlabLineContinuationComment	'\.\{3}.*$'			contains=matlabLineContinuation,matlabTodo,@Spell	
"     ... Normal Comment ...
syn match matlabComment			"%.*$"				contains=matlabTodo,matlabTitle,matlabMLint,@Spell	
"     ... Block Comment ...
syn region matlabBlockComment		start=+%{+	end=+%}+	contains=matlabBlockComment,@Spell fold
syn region matlabCommentRegion		start=+\%(^\s*\)\@<=%\%(%\s\)\@!+ skip=+^\s*%\%($\|[^%]\|%\S\)+ end=+^+ transparent contains=@Spell,@matlabCommentCluster fold
"     ... Comment Cluster ...
...
if version >= 508 || !exists("did_matlab_syntax_inits")
   ...
   HiLink matlabWord			Normal		
   HiLink matlabStatement		Statement
   HiLink matlabFunction		Statement
   "
   HiLink matlabScope			Keyword		
   ...
   HiLink matlabType			Type		
   HiLink matlabIdentifier		Identifier	
   HiLink matlabFunc			Function

   " ... Matlab Class ...
   HiLink matlabObjectOriented		Keyword		
   HiLink matlabClassdef		Keyword		
   HiLink matlabClass			Keyword		
   HiLink matlabClassArgument		Identifier	
   HiLink matlabClassAttribute		Boolean		

   " ... End Definitions ...
   HiLink matlabProceduralEnd		Statement
   HiLink matlabIndexEnd		Identifier	
   HiLink matlabOperatorEnd		Function	

   " ... System Commands and PreProc ...
   HiLink matlabSystemCommand		PreProc		
   ...
   HiLink matlabIncompleteString	WarningMsg	
   HiLink matlabSprintfFormat		SpecialChar	
   HiLink matlabTexFormat		Character	
   ...
   " ... Comments ...
   HiLink matlabTitle			Title		
   "
   HiLink matlabComment			Comment
   HiLink matlabBlockComment		Comment
   HiLink matlabLineContinuationComment	Comment		
   "
   HiLink matlabMLint			SpecialComment	
   "
   ...
   HiLink matlabStructDot		matlabOperator	
   HiLink matlabColon			matlabOperator	
   HiLink matlabFunctionHandle		matlabOperator	
   HiLink matlabQuestionMark		matlabOperator	
   "
   ...
   " ... Miscellaneous ...
   HiLink matlabColours			Identifier	
   HiLink matlabDebug			Debug		
